...
int main(int argc, char** argv){
    ...
    HitTreeCoordinateConverter *converter = new HitTreeCoordinateConverter();
    ...
    if(argc > 4){
        TString delete_fluo_option = argv[4];
        if(delete_fluo_option == "DeleteFluo"){
            TString header = "deletefluo_global_";
            converter->SetOutfileHeader(header);
            converter->SetDeleteFluo(true);
        }
    }
...
void DescribeInputParameters(){
    cout << "1 : rootfilename (including hittree)" << endl;
    cout << "2 : parameter file (.json)" << endl;
    cout << "3 : cameraname(ex. 1 or cam1 or cc1 or camera1)" << endl;
    cout << "4 : optional/if this is \"DeleteFluo\", then fluo line hits are removed." << endl;
}
